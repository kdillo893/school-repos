...
static trace_t *read_trace(char *tracedir, char *filename) {
  ...
  if ((tracefile = fopen(path, "r")) == NULL) {
    //truncate path to Maxline - 29 length to fit sprint in 1024 size
    char pathMod[MAXLINE-29];
    strcpy(pathMod, path);

    sprintf(msg, "Could not open %s in read_trace", pathMod);
    unix_error(msg);
  }
